...
onready var pathfinding: PathfindAstar = get_node_or_null(pathfinding_path)		# No necesariamente va a conocer un nodo pathfinding
...
func _physics_process(delta: float) -> void:
	if target != null:		
		...
		if raycast.is_colliding() && raycast.get_collider() == target:
			# Si hay un target y puede verse, dejar de moverse y me prepara para disparar
			path = []
	...
	if !path.empty():
		...
		velocity = (velocity + global_position.direction_to(next_point) * speed).clamped(max_speed)
	else:	
